<?php
	...
	if(isset($_POST["add-product"])){
		...
		$thisdate = date("Y-m-d H:i:s");
			 ...
			 if ($conn->query($products_insert)===TRUE){
				$pid = $conn->insert_id;
				...
				header('location: ../admin/product_more.php?product='.$pid);
				 ...
				 $err= "Unable to upload Item. <br/>" . $conn->error;
		         $_SESSION["error"] = $err;
		...
		$res = $conn->query("SELECT * FROM prod_img WHERE img_id='$id'");
		$row = $res->fetch_assoc();
	...
	if(isset($_POST["image-id-edit"])){
		$id = mysqli_real_escape_string($conn, $_POST["image-id-edit"]);
		$image = $_FILES['photos']['tmp_name'];
        $imgContent = addslashes(file_get_contents($image));
		...
		if($new_name === TRUE){
			 ...
			 if ($conn->query($products_insert)===TRUE){
				...
				$_SESSION["success"] = $msg;
				 ...
				 $err= "Unable to upload Image. <br/>" . $conn->error;
		         $_SESSION["error"] = $err;
			 }
		}else{ 
			$err= "An error occured. Please try again. <br/>" . $conn->error;
		    $_SESSION["error"] = $err;
		}
		...
		$id = mysqli_real_escape_string($conn, $_POST["status-edit0"]);
		$state = 1;
	...
	elseif(isset($_POST["status-edit1"])){
		$id = mysqli_real_escape_string($conn, $_POST["status-edit1"]);
		$state = 0;
